...
public:
	...
	bool insert(unsigned int position, Equipment* element);

	// Operator=
	const Vector& operator= (const Vector & theOther);

	// operator[]
	Equipment* operator [](unsigned int position);
	const Equipment* operator [](unsigned int position)const;
};
